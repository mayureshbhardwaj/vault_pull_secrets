...
# Azure SDK imports (optional - only loaded if Azure Key Vault upload is enabled)
try:
...
_AZURE_KEYVAULT_CLIENT = None
_KEYVAULT_METADATA = []  # List of secrets uploaded to Key Vault


def get_keyvault_client(key_vault_url: str) -> Optional[SecretClient]:
    """Get Azure Key Vault client using managed identity or service principal.

    Args:
        key_vault_url: Azure Key Vault URL (e.g., https://kv-prod.vault.azure.net)

    Returns:
        SecretClient instance or None if Azure SDK not available
    """
    global _AZURE_KEYVAULT_CLIENT

    if not AZURE_SDK_AVAILABLE:
        return None

    if _AZURE_KEYVAULT_CLIENT is None:
        try:
            ...
            _AZURE_KEYVAULT_CLIENT = SecretClient(vault_url=key_vault_url, credential=credential)
        except Exception as e:
            print(f"⚠️  Failed to create Key Vault client: {type(e).__name__}")
            return None

    return _AZURE_KEYVAULT_CLIENT


def should_upload_to_keyvault(secret_name: str, secret_value: str) -> bool:
    """Determine if a secret should be uploaded to Azure Key Vault.

    Binary secrets (JKS files, certificates, etc.) that are base64-encoded
    should be stored in Azure Key Vault and mounted as files in Container Apps.

    Args:
        secret_name: Name of the secret
        secret_value: Secret value from Vault

    Returns:
        True if secret should go to Azure Key Vault, False for encrypted file
    """
    # Check if value is base64-encoded
    if not is_base64(secret_value):
        return False

    # Check if it's likely a binary secret based on naming patterns
    binary_patterns = [
        'jks', 'keystore', 'truststore',  # Java KeyStore
        'p12', 'pfx',  # PKCS#12
        'key', 'crt', 'cer', 'pem',  # Certificates
        'cert', 'certificate',
    ]
...
def upload_to_azure_keyvault(
    secret_name: str,
    secret_value: str,
    key_vault_url: str,
    managed_identity_id: Optional[str] = None
) -> Optional[Dict[str, str]]:
    """Upload secret to Azure Key Vault (idempotent).

    Args:
        secret_name: Name of the secret
        secret_value: Base64-encoded secret value
        key_vault_url: Azure Key Vault URL
        managed_identity_id: Managed identity resource ID (for metadata)

    Returns:
        Metadata dictionary with name and keyVaultUrl, or None if failed
    """
    if not AZURE_SDK_AVAILABLE:
        print(f"⚠️  Azure SDK not available, cannot upload secret (hash: {get_secret_hash(secret_name)})")
        return None

    client = get_keyvault_client(key_vault_url)
    if not client:
        return None

    try:
        # Check if secret already exists and is unchanged (idempotency)
        try:
            existing_secret = client.get_secret(secret_name)
            # Compare hash to avoid unnecessary updates
            existing_hash = hashlib.sha256(existing_secret.value.encode()).hexdigest()
            new_hash = hashlib.sha256(secret_value.encode()).hexdigest()

            if existing_hash == new_hash:
                print(f"⏭️  Secret unchanged in Key Vault, skipping upload (hash: {get_secret_hash(secret_name)})")
                # Return metadata even though we didn't upload
                return {
                    'name': secret_name,
                    'keyVaultUrl': f"{key_vault_url}/secrets/{secret_name}",
                    'identity': managed_identity_id
                }
        except ResourceNotFoundError:
            # Secret doesn't exist yet, will upload below
            pass
        ...
        client.set_secret(
            name=secret_name,
            value=secret_value,
            content_type="application/x-java-keystore"  # Hint for binary content
        )

        print(f"✅ Uploaded to Azure Key Vault (hash: {get_secret_hash(secret_name)})")
        ...
        _KEYVAULT_METADATA.append(metadata)
        return metadata

    except Exception as e:
        print(f"❌ Failed to upload to Key Vault (hash: {get_secret_hash(secret_name)}): {type(e).__name__}: {str(e)}")
        return None


def get_keyvault_metadata() -> List[Dict[str, str]]:
    """Get list of secrets uploaded to Azure Key Vault during this run.

    Returns:
        List of metadata dictionaries
    """
    return _KEYVAULT_METADATA
...
def pull_secrets_batch(
    ...
    Automatically routes secrets based on type:
    - Binary secrets (JKS, certs) → Azure Key Vault
    - Regular secrets → Encrypted files

    Args:
        path: Vault path
        keys: List of keys to extract from the path
        output_dir: Directory to save secrets
        key_vault_url: Azure Key Vault URL (optional, enables auto-upload)
        managed_identity_id: Managed identity resource ID (optional)
        ...
        if result.returncode == 0:
            ...
            for key in keys:
                if key in secret_data:
                    ...
                    # Route secret based on type
                    if key_vault_url and should_upload_to_keyvault(key, processed_value):
                        # Binary secret → Azure Key Vault
                        print(f"   🔑 Routing to Azure Key Vault (binary detected, hash: {get_secret_hash(key)})")
                        metadata = upload_to_azure_keyvault(
                            secret_name=key,
                            secret_value=processed_value,
                            key_vault_url=key_vault_url,
                            managed_identity_id=managed_identity_id
                        )
                        results[key] = metadata is not None
                    else:
                        # Regular secret → Encrypted file
                        encrypted_data = encrypt_secret(processed_value)
...
def main():
    ...
    # Extract global config for Key Vault integration
    global_config = config.get('global', {})
    managed_identity_id = global_config.get('managedIdentityId', '')

    # Azure Key Vault integration (read from config)
    key_vault_url = global_config.get('azureKeyVaultUrl', '')
    enable_binary_upload = global_config.get('enableBinaryUpload', True)

    # Display Key Vault configuration
    if key_vault_url and enable_binary_upload:
        print(f"Azure Key Vault: {key_vault_url}")
        print(f"Binary Upload: Enabled")
    else:
        print(f"Binary Upload: Disabled")
    print("=====================\n")

    # Disable Key Vault upload if URL not provided or explicitly disabled
    if not key_vault_url or not enable_binary_upload:
        key_vault_url = None
    ...
    # Output Key Vault metadata for deployment action
    keyvault_metadata = get_keyvault_metadata()
    if keyvault_metadata:
        print(f"\n📦 Key Vault Secrets ({len(keyvault_metadata)} uploaded):")
        # Write metadata to file for GitHub Actions to read
        metadata_file = 'secrets/keyvault-refs.json'
        with open(metadata_file, 'w') as f:
            json.dump(keyvault_metadata, f, indent=2)
        print(f"   Metadata written to: {metadata_file}")
        for meta in keyvault_metadata:
            print(f"   - {meta['name']} → {meta['keyVaultUrl']}")
